...
def _submit_extraction(base_url, filename, payload, pattern="*.json", headers=None):
    return _http_multipart(
        base_url,
        "/extractions/",
        {"archive": (filename, payload), "pattern": pattern},
        headers=headers,
    )


def _poll_status(base_url, job_id, endpoint_prefix, timeout=10.0, interval=0.1):
    deadline = time.time() + timeout
    while time.time() < deadline:
        status, body = _http_json("GET", base_url, f"/{endpoint_prefix}/{job_id}")
        assert status == 200
...
def test_live_extraction_submit_returns_202_and_job_id(base_url):
    status, body = _submit_extraction(base_url, "sample.zip", _make_zip_bytes())

    assert status == 202
    assert "job_id" in body


def test_live_extraction_submit_and_poll_status(base_url):
    status, body = _submit_extraction(base_url, "sample.zip", _make_zip_bytes())
    assert status == 202
    job_id = body["job_id"]
...
def test_live_extraction_results_after_completed(base_url):
    status, body = _submit_extraction(base_url, "sample.zip", _make_zip_bytes())
    assert status == 202
    job_id = body["job_id"]
    ...
    rs, rb = _http_json("GET", base_url, f"/extractions/{job_id}/results")
...
def test_live_extraction_results_pagination(base_url):
    status, body = _submit_extraction(base_url, "sample.zip", _make_zip_bytes())
    assert status == 202
    job_id = body["job_id"]
    ...
    assert rb["limit"] == 1
    assert rb["offset"] == 0
    assert len(rb["files"]) <= 1


def test_live_extraction_results_while_processing_returns_conflict(base_url):
    status, body = _submit_extraction(base_url, "sample.zip", _make_zip_bytes())
    assert status == 202
    job_id = body["job_id"]
...
def test_live_extraction_missing_file_returns_validation_error(base_url):
    boundary = uuid4().hex
    body = f"--{boundary}--\r\n".encode()

    req = request.Request(
        url=f"{base_url.rstrip('/')}/extractions/",
        data=body,
...
def test_live_extraction_job_not_found(base_url):
    status, body = _http_json("GET", base_url, "/extractions/non-existent-job-id")
...
def test_live_extraction_request_id_propagation(base_url):

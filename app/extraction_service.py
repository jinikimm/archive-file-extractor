...
def user_bulk_import_task(csv_content):
    """
    Import users from CSV content
    CSV format: email,name

    Returns:
        dict with import statistics (created, updated, errors)
    """
    result = {
        "status": "success",
        "created": 0,
    ...
    try:
            ...
            try:
                ...
                if not email or not name:
                    result["errors"].append(
                        {"row": row_num, "error": "Email and name are required"}
                    )
                    continue
        ...
        result["status"] = "error"
        result["errors"].append({"row": "general", "error": str(e)})

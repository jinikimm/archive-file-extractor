...
def extract_archive(file_path):
    if not is_archive(file_path):
        raise ValidationError(
            ...
            details=[
                {"field": "archive", "message": f"Unsupported format: {file_path}"}
            ],
        )

    try:
        extract_dir = os.path.join(
            os.path.dirname(file_path), f"{os.path.basename(file_path)}_extracted"
        )
        ...
        raise ArchiveError(
            ...
            details=[
                {
                    "field": "archive",
                    "message": f"Failed to extract: {file_path} - {str(e)}",
                }
            ],
        )
...
def extract_all_archives_parrel(file_path, max_depth=10, max_worker=10):
    ...
    with ThreadPoolExecutor(max_workers=max_worker) as pool:
        futures = {
            pool.submit(extract_archive, file_path): {
                "file_path": file_path,
                "depth": 0,
            }
        }
            ...
            for future in done:
                ...
                for f in extracted_files:
                    if is_archive(f["file_name"]) and archive_depth < max_depth:
                        futures[pool.submit(extract_archive, f["full_path"])] = {
